...
public interface LearnerDashboardRepository extends AbstractRepository{
	...
	@Query("select count(h), avg(h.budget.amount), stddev(h.budget.amount), min(h.budget.amount), max(h.budget.amount) from HelpRequest h where h.status = :status group by h.budget.currency")
